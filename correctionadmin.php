...
// Traitement du formulaire d'update en haut du fichier
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_user') {
    $user_id = intval($_POST['user_id']);
    $full_name = explode(' ', $_POST['full_name'], 2);
    $name= htmlspecialchars($full_name[0]);
    $surname_user = isset($full_name[1]) ? $full_name[1] : '';
    ...
    try {
        ...
        // SOLUTION : Redirection après POST pour éviter le problème de refresh
        header("Location: " . $_SERVER['PHP_SELF'] . "?success=1&tab=users");
        exit();
        
    } catch (PDOException $e) {
        $error_message = "Erreur lors de la modification : " . $e->getMessage();
        header("Location: " . $_SERVER['PHP_SELF'] . "?error=" . urlencode($e->getMessage()) . "&tab=users");
        exit();
    }
}
                            ...
                            <tbody>
                                <?php foreach ($utilisateurs as $utilisateur): ?>
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="user-avatar me-3"><?= strtoupper(substr($utilisateur['full_name'], 0, 2)) ?></div>
                                                <div>
                                                    <strong><?= htmlspecialchars($utilisateur['full_name']) ?></strong>
                                                    <br>
                                                    <small class="text-muted"><?= htmlspecialchars($utilisateur['role_user']) ?></small>
                                                </div>
                                            </div>
                                        </td>
                                        <td><?= htmlspecialchars($utilisateur['email_user']) ?></td>
                                        <td>
                                            <span class="badge bg-primary"><?= htmlspecialchars($utilisateur['role_user']) ?></span>
                                        </td>
                                        <td><?= htmlspecialchars($utilisateur['subscriptiondate_user']) ?></td>
                                        <td>
                                            <span class="badge bg-success"><?= htmlspecialchars($utilisateur['status_user']) ?></span>
                                        </td>
                                        <td>
                                            ...
                                            <button class="btn btn-warning btn-action edit-user-btn" 
                                                    title="Modifier"
                                                    data-user-id="<?= $utilisateur['id_user'] ?>"
                                                    data-user-name="<?= htmlspecialchars($utilisateur['full_name']) ?>"
                                                    data-user-email="<?= htmlspecialchars($utilisateur['email_user']) ?>"
                                                    data-user-role="<?= htmlspecialchars($utilisateur['role_user']) ?>"
                                                    data-user-status="<?= htmlspecialchars($utilisateur['status_user']) ?>">
